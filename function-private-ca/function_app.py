...
def create_signed_certificate_simple(subject, public_key, issuer_cert, validity_days):
    """
    Creates a signed certificate using a simplified approach.
    This is a fallback method when Key Vault HSM signing is not available.
    For production with HSM, use the sign_certificate_with_keyvault function.
    """
    # Generate temporary key for signing (in production, this would be the CA key from KV)
    temp_ca_key = rsa.generate_private_key(
    ...
    cert_builder = cert_builder.issuer_name(issuer_cert.subject)

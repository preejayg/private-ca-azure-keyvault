...
# Renew a device certificate (LOCAL VERSION)
# Usage: ./renew-certificate-local.sh <certificate-name> [validity_days] [auto_revoke] [revocation_reason] [generate_csr]
#
# PREREQUISITES:
...
echo "========================================="
echo "Renew Device Certificate (Local)"
echo "========================================="
...
if [ "$GENERATE_CSR" = "true" ]; then
    echo "[2/4] Generating new CSR with OpenSSL..."
    ...
    echo "[2/4] Skipping CSR generation (reusing existing subject/key)"
    echo ""
fi

# Build JSON payload
echo "[3/4] Building renewal request..."
if [ -n "$CSR_DATA" ]; then
    # With CSR
    PAYLOAD=$(jq -n \
...
# Renew certificate
echo "[4/4] Renewing certificate..."

RESPONSE=$(curl -s -X POST "https://${FUNCTION_HOST}/api/renew-certificate" \
